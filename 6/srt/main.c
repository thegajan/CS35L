...
scene_t scene;
float scaled_color_g[height][width][3];
int nthreads;
...
void * ray(void * t_id){
    ...
    for( int px=*(int *)t_id; px<width; px += nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            scaled_color_g[py][px][0] = scaled_color[0];
            scaled_color_g[py][px][1] = scaled_color[1];
            scaled_color_g[py][px][2] = scaled_color[2];
        }
    }
  return NULL;
}
...
main( int argc, char **argv )
{
    ...
    for(x = 0; x < nthreads; x++){
      t_id[x] = x;
      ret = pthread_create(&t[x], NULL, ray, (void *) (&t_id[x]));
      if(ret){
    ...
    for(x = 0; x < nthreads; x++){
      ret = pthread_join(t[x], NULL);
      if(ret){
        fprintf(stderr, "Error joining thread. Error code is %d\n", ret);
        exit(-1);
      }
    }
    ...
    for (i = 0; i < width; i++){
      for (j = 0; j < height; j++)
        printf("%.0f %.0f %.0f\n", scaled_color_g[j][i][0], scaled_color_g[j][i][1], scaled_color_g[j][i][2]); 
      printf("\n");
    }
